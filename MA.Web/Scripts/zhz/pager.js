    ...
    */
    this.render = function () {
        ...
        if (self._pageCount > 1) {
            if (self.index != 1) {
                str += '<a href="javascript://1"><span class="btn btn-white">首页</span></a>';
                str += '<a href="javascript://' + (self.index - 1) + '"><span class="btn btn-white">上一页</span></a>';
            } else {
                str += '<span class="btn btn-white">首页</span>';
                str += '<span class="btn btn-white">上一页</span>';
            }
        }
        for (var i = self._start; i <= self._end; i++) {
            if (i == this.index) {
                str += '<span class="btn btn-white  active">' + i + "</span>";
            } else {
                str += '<a href="javascript://' + i + '"><span class="btn btn-white">' + i + '</span></a>';
            }
        }
        if (self._pageCount > 1) {
            if (self.index != self._pageCount) {
                str += '<a href="javascript://' + (self.index + 1) + '"><span class="btn btn-white">下一页</span></a>';
                str += '<a href="javascript://' + self._pageCount + '"><span class="btn btn-white">尾页</span></a>';
            } else {
                str += '<span class="btn btn-white">下一页</span>';
                str += '<span class="btn btn-white">尾页</span>';
            }
        }
